...
function ensureLocalHttpsCerts(userDataPath: string, homePath: string): void {
  ...
  const caPath = join(certDir, 'rootCA.pem');
  ...
  if (existsSync(certPath) && existsSync(keyPath)) {
    ...
    process.env.HTTPS_CA_PATH = process.env.HTTPS_CA_PATH || (existsSync(caPath) ? caPath : '');
  ...
  try {
    const loginKeychain = join(homePath, 'Library', 'Keychains', 'login.keychain-db');
...
async function loadDesktopEnv() {
  ...
  try {
    ...
    ensureLocalHttpsCerts(userDataPath, homePath);
