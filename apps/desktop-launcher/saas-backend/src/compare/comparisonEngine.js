...
class ComparisonEngine {
    ...
    try {
      ...
      if (!matchedElement) {
        return {
          ...
          component: {
            id: sanitizedComponent.id,
            name: this.trimString(sanitizedComponent.name, 100),
            type: sanitizedComponent.type
          },
          element: null,
          webElement: null,
          status: 'no_match',
      ...
      const elementSummary = this.summarizeWebElement(matchedElement);
  ...
  summarizeWebElement(webElement) {
    if (!webElement || typeof webElement !== 'object') return null;

    const className = webElement.className || webElement.class || '';
    const selector = webElement.selector || '';
    const tagName = webElement.type || webElement.tagName || webElement.tag || this.inferTagFromSelector(selector);
    const domId = webElement.attributes?.id || webElement.domId || this.filterSyntheticElementId(webElement.id);
    const classes = Array.isArray(webElement.classes)
      ? webElement.classes
      : className
  ...
  filterSyntheticElementId(id) {
    if (!id || typeof id !== 'string') return null;
    return id.startsWith('element-') ? null : id;
  }
