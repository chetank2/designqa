...
let siteProfilesRepository = null;
...
/**
 * Get site profiles repository (adapter that wraps storage provider)
 * @returns {Object|null} Site profiles adapter with repository-like interface
 */
export function getSiteProfilesRepository() {
  try {
    const storageProvider = getStorageProvider();
    if (storageProvider && typeof storageProvider.listSiteProfiles === 'function') {
      if (!siteProfilesRepository) {
        siteProfilesRepository = {
          async list(filters = {}) {
            return await storageProvider.listSiteProfiles(filters);
          },
          async create(data) {
            return await storageProvider.saveSiteProfile(data, {});
          },
          async findById(id) {
            return await storageProvider.getSiteProfile(id);
          },
          async update(id, data) {
            const existing = await storageProvider.getSiteProfile(id);
            if (!existing) throw new Error('Site profile not found');
            const updated = { ...existing, ...data };
            return await storageProvider.saveSiteProfile(updated, { id });
          },
          async delete(id) {
            return await storageProvider.deleteSiteProfile(id);
          }
        };
      }
      return siteProfilesRepository;
    }
    console.warn('Storage provider does not support site profile operations');
    return null;
  } catch (error) {
    console.warn('Failed to get storage provider for site profiles:', error.message);
    return null;
  }
}
