...
export class LocalStorageProvider extends StorageProvider {
  constructor() {
    ...
    this.siteProfilesDir = getSiteProfilesDir();
  ...
  async saveReport(reportData, metadata) {
    ...
    // Persist metadata for future lookups (comparisonId, title, timestamps)
    const metadataPath = path.join(this.reportsDir, `${reportId}.meta.json`);
    try {
      await fsPromises.writeFile(metadataPath, JSON.stringify(reportMetadata, null, 2), 'utf8');
    } catch (metaError) {
      // Non-fatal: report file exists even if metadata write fails
      console.warn(`⚠️ Failed to write report metadata for ${reportId}:`, metaError.message);
    }
  ...
  async getReport(reportId) {
    ...
    const reportFile = files.find(file =>
      file.startsWith(reportId) &&
      !file.endsWith('.meta.json') &&
      /\.(html|json|pdf|csv)$/.test(file)
    );
    ...
    try {
      ...
      storedMetadata = JSON.parse(metadataRaw);
    } catch (metaError) {
      storedMetadata = null;
    }
    ...
    return {
      ...
      metadata: {
        id: reportId,
        title: storedMetadata?.title || `Report ${reportId}`,
        comparisonId: storedMetadata?.comparisonId || null,
        format,
        url: `/reports/${reportFile}`,
        fileSize: stats.size,
        createdAt: storedMetadata?.createdAt || stats.birthtime.toISOString()
      }
    };
  }
  ...
  async listReports(filters = {}) {
    ...
    for (const file of files) {
      ...
      if (filters.comparisonId) {
        const comparisonMatch = storedMetadata?.comparisonId === filters.comparisonId;
    ...
    const limit = Math.max(0, parseInt(filters.limit, 10) || reports.length);
  ...
  // Site profile storage methods for local mode
  async saveSiteProfile(profileData, metadata) {
    const {
      name,
      domain,
      enabled = true,
    ...
    const profilesDir = path.join(this.baseDir, 'site-profiles');
    if (!fs.existsSync(profilesDir)) {
      fs.mkdirSync(profilesDir, { recursive: true });
    }

    const profileId = metadata?.id || `profile_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    let existing = null;
    if (metadata?.id) {
      try {
        existing = await this.getSiteProfile(profileId);
      } catch {
        existing = null;
      }
    }
    ...
    const parsedTimeouts = {
      extraction: Number.isFinite(Number(timeouts.extraction))
        ? Number(timeouts.extraction)
        : existing?.timeouts?.extraction || null,
      stability: Number.isFinite(Number(timeouts.stability))
        ? Number(timeouts.stability)
        : existing?.timeouts?.stability || null,
      navigation: Number.isFinite(Number(timeouts.navigation))
        ? Number(timeouts.navigation)
        : existing?.timeouts?.navigation || null
    };
    ...
    const stored = {
      ...
      resourceBlocking: resourceBlocking || existing?.resourceBlocking || 'auto',
      waitForSelector: waitForSelector?.trim() || existing?.waitForSelector || null,
    ...
    try {
      await fsPromises.writeFile(filePath, JSON.stringify(stored, null, 2), 'utf8');
    } catch (error) {
      throw new Error(`Failed to save site profile: ${error.message}`);
    }

    return stored;
  }

  async listSiteProfiles(filters = {}) {
    const profilesDir = path.join(this.baseDir, 'site-profiles');
    if (!fs.existsSync(profilesDir)) {
      return [];
    }
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (filters.enabled !== undefined && Boolean(profile.enabled) !== Boolean(filters.enabled)) {
          continue;
        }
    ...
    profiles.sort((a, b) => new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at));
    return profiles;
  }

  async getSiteProfile(profileId) {
    const profilesDir = path.join(this.baseDir, 'site-profiles');
    const filePath = path.join(profilesDir, `${profileId}.json`);
    if (!fs.existsSync(filePath)) {
      throw new Error(`Site profile not found: ${profileId}`);
    }
    const data = await fsPromises.readFile(filePath, 'utf8');
    return JSON.parse(data);
  }

  async deleteSiteProfile(profileId) {
    const profilesDir = path.join(this.baseDir, 'site-profiles');
    const filePath = path.join(profilesDir, `${profileId}.json`);
    if (!fs.existsSync(filePath)) {
      return false;
