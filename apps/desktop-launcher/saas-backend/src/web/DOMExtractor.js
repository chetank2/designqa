...
export function extractDOMElements(pageUrl) {
    ...
    const addElement = (element, selectorIndex, elemIndex) => {
        ...
        // Much more permissive criteria for multi-page applications like FreightTiger
        const hasClassName = element.className && element.className.trim().length > 0;
        const isSemanticTag = ['header', 'nav', 'main', 'section', 'article', 'aside', 'footer', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'div', 'span', 'ul', 'ol', 'li', 'button', 'input', 'form', 'table', 'tr', 'td', 'th'].includes(tag);
        const hasMinimumSize = rect.width >= 5 && rect.height >= 5; // Further reduced minimum size
        const isVisible = rect.width > 0 && rect.height > 0; // Basic visibility check
        const hasAnyContent = element.innerText?.trim().length > 0 || element.innerHTML?.includes('<') || hasClassName;

        // Much more inclusive logic - accept almost any element that might contain useful design information
        if (hasText || hasBackground || hasBorder || isLarge || isImage || isInteractive ||
            (isSemanticTag && hasMinimumSize && isVisible) || // Remove className requirement
            hasAnyContent || // Include any element with content or className
            (rect.width >= 20 && rect.height >= 20)) { // Include larger elements regardless
            const elementData = {
