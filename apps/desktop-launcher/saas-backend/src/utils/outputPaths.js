...
function resolveCustomDir() {
  ...
  if (process.env.DESIGNQA_USER_DATA_DIR) {
    ...
    if (hasLegacyDataDir(resolved) && !hasLegacyDataDir(dataDir)) {
      return resolved;
    }
    return dataDir;
  }
...
function hasLegacyDataDir(baseDir) {
  try {
    const markers = [
      'credentials',
      'design-systems',
    ...
    return markers.some((name) => fs.existsSync(path.join(baseDir, name)));
  } catch {
    return false;
  }
}
...
export function getSiteProfilesDir() {
  return resolveSubDir('site-profiles');
}
