...
const getStorage = async (req) => {
  ...
  return getStorageProvider(req.user?.id);
};

const resolveReportMetadata = async (req, id) => {
  if (req.dbServices?.reports) {
    try {
      const report = await req.dbServices.reports.findById(id);
      if (report) {
  ...
  try {
    ...
    return { report: reportData.metadata, data: reportData.data };
  } catch (storageError) {
    logger.warn('Storage provider failed to get report:', storageError.message);
  }

  return { report: null, data: null };
};

const findReportHtmlByPath = async (reportPath) => {
  if (!reportPath) return null;
  const reportFileName = path.basename(
    reportPath.startsWith('/reports/') ? reportPath.substring('/reports/'.length) : reportPath
  );
  const { getOutputBaseDir } = await import('../utils/outputPaths.js');
  const outputBaseDir = getOutputBaseDir();

  const possibleDirs = [
    path.join(outputBaseDir, 'reports'),
    path.join(process.cwd(), 'output', 'reports'),
    path.join(process.cwd(), 'apps', 'saas-backend', 'output', 'reports')
  ];

  for (const dir of possibleDirs) {
    const filePath = path.join(dir, reportFileName);
    try {
...
router.get('/:id/export-dev-csv', async (req, res) => {
  try {
    ...
    const { report } = await resolveReportMetadata(req, id);
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { report, data } = await resolveReportMetadata(req, id);
    ...
    if (req.headers.accept && req.headers.accept.includes('text/html')) {
      ...
      try {
        ...
        logger.error('Storage provider failed to get report HTML:', storageError.message);
      }
    }
...
router.get('/:id/view', async (req, res) => {
  try {
    ...
    try {
      ...
      const { report } = await resolveReportMetadata(req, id);
      const comparisonId = report?.comparisonId;

      // Try to regenerate report from comparison data if available
      if (comparisonId && req.dbServices?.comparisons) {
        try {
          ...
          if (comparisonData) {
            ...
            try {
              ...
              logger.info(`Regenerated and saved report for comparison ${comparisonId}`);
            } catch (saveError) {
...
router.post('/save', async (req, res) => {
  try {
    ...
    if (!comparisonId && !reportData && !reportPath) {
      return res.status(400).json({
        success: false,
        error: 'comparisonId, reportData, or reportPath is required'
      });
    }
    ...
    // If reportPath is provided, read and save that file
    else if (reportPath) {
      const reportHtml = await findReportHtmlByPath(reportPath);
      if (!reportHtml) {
        return res.status(404).json({
          success: false,
          error: 'Report file not found at provided path'
        });
      }

      const storage = await getStorage(req);
      reportEntry = await storage.saveReport(reportHtml, {
        comparisonId,
        title: title || `Report - ${new Date().toLocaleDateString()}`,
        format,
        userId: req.user?.id
    ...
    // If only comparisonId is provided, generate a report from comparison data
    else if (comparisonId) {
      if (!req.dbServices?.comparisons) {
        return res.status(400).json({
          success: false,
          error: 'Comparison data is not available to generate a report'
        });
      }
    ...
    if (req.dbServices?.reports) {
      try {
        await req.dbServices.reports.create({
          id: reportEntry.id,
          userId: req.user?.id,
          comparisonId: reportEntry.comparisonId || comparisonId || null,
